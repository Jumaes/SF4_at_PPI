...
minhits=2 # Only those boxes of CA atoms count as hits, which have MORE than 'minhits' SF4 template structure CA's in it. I.e. minhits=2 selects boxes with 3 or more hits.
...
capadding=0.5 # Padding, which is allowed for CA atom in question to miss the CA box in any direction. Note that multiple boxes might be found for each CA atom.
fepadding=0.5 # Padding around the "average" Fe atom of the SF4 used in the search for clashes of the new SF4 with the backbone of the scaffold structure.
npadding=0.5 #Padding, which is allowed for N atoms in question to miss the N box in any direction.
cpadding=0.5 #Padding, which is allowed for C atoms in question to miss the C box in any direction.
...
scaffoldlibfolder='/media/tezcanlab/Data/Julian/SF4project/1GTP/'
...
def checking_scaffold_protein(scaffold):
    ...
    try: # make subfolder in subfolder to store the actually SF4 clashing structures to keep everything a bit more tidy.
        ...
        pass
    cmd.select('start_ca','name ca and (resi 205-221) and chain A') # In case you wanna limit search to some of them.
                ...
                if box[1][0]-capadding<stored.query_resi_coor[0][0]<box[1][0]+edge+capadding and box[1][1]-capadding<stored.query_resi_coor[0][1]<box[1][1]+edge+capadding and box[1][2]-capadding<stored.query_resi_coor[0][2]<box[1][2]+edge+capadding:
                    ...
                    for nbox in narray:
                        if nbox[0]==box[0]: #Find the nbox, which is associated with the CA box, where we had a hit.
                            ...
                            if nbox[4]-npadding<stored.query_resi_Ncoor[0][0]<nbox[3]+npadding and nbox[6]-npadding<stored.query_resi_Ncoor[0][1]<nbox[5]+npadding and nbox[8]-npadding<stored.query_resi_Ncoor[0][2]<nbox[7]+npadding: #TODO: When running on 3sbq, this line throws a list index out of range error... dunno what is wrong there.
                                    ...
                                    for cbox in carray:
                                        if cbox[0]==nbox[0]:
                                            ...
                                            if cbox[4]-cpadding<stored.query_resi_Ccoor[0][0]<cbox[3]+cpadding and cbox[6]-cpadding<stored.query_resi_Ccoor[0][1]<cbox[5]+cpadding and cbox[8]-cpadding<stored.query_resi_Ccoor[0][2]<cbox[7]+cpadding:
